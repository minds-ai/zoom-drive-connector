...
class ConfigInterface:
  ...
  def __create_interfaces(self):
    ...
    zoom = conf_dict["zoom"]
    print(zoom['meetings'])
    self.zoom_conf = ZoomConfig(zoom['key'], zoom['secret'], zoom['username'], zoom['password'],
                                zoom['delete'], zoom['meetings'])
    drive = conf_dict["drive"]
    self.drive_conf = DriveConfig(drive['credentials_json'], drive['client_secret_json'],
                                  drive['folder_id'])
